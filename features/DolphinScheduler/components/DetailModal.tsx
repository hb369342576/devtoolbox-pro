            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-200 dark:border-slate-700" onClick={e => e.stopPropagation()}>
                <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/80">
                    <h3 className="font-bold text-lg text-slate-800 dark:text-white flex items-center">
                        ...
                        {t('dolphinScheduler.workflowDetails')}
                    </h3>
                ...
                <div className="p-6 max-h-[70vh] overflow-y-auto">
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('dolphinScheduler.workflowName')}</label>
                                <p className="text-slate-800 dark:text-white font-medium">{process.name}</p>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('dolphinScheduler.version')}</label>
                                <p className="text-slate-800 dark:text-white">v{process.version}</p>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('dolphinScheduler.releaseState')}</label>
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('dolphinScheduler.scheduleState')}</label>
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('dolphinScheduler.modifiedBy')}</label>
                                <p className="text-slate-600 dark:text-slate-300">{process.userName || process.modifyBy || '-'}</p>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('dolphinScheduler.updatedTime')}</label>
                                <p className="text-slate-600 dark:text-slate-300">{new Date(process.updateTime).toLocaleString()}</p>
                            </div>
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">{t('common.description')}</label>
                            <p className="text-slate-600 dark:text-slate-300 mt-1">{process.description || t('dolphinScheduler.noDescription')}</p>
                        </div>
                        <div>
