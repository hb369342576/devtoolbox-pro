...
export const defaultSettingsTemplate = {
    common: {
        ...
        retryTimes: 0,
        retryInterval: 1,
        defaultEnvCode: undefined as number | undefined,
    },
    nodes: {
        k8s: {
...
const DB_TYPES = ['MYSQL','DORIS','POSTGRESQL','ORACLE','SQLSERVER','HIVE','SPARK','CLICKHOUSE','STARROCKS','PRESTO','TRINO'];
...
export const GlobalSettingsModal: React.FC<GlobalSettingsModalProps> = ({ show, onClose, onSave }) => {
    ...
    const inputClass = "w-full max-w-xs px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-slate-200 text-sm";
    const labelClass = "block text-slate-700 dark:text-slate-300 text-sm font-medium mb-1";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden border border-slate-200 dark:border-slate-700 flex flex-col" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="flex-1 p-6 overflow-y-auto">
                        {activeTab === 'common' && (
                            <div className="space-y-5 animate-in fade-in slide-in-from-right-4">
                                ...
                                <div>
                                    <label className={labelClass}>{t('dolphinScheduler.globalSettings.labels.defaultTimeout')}</label>
                                    <input type="number" min="0" value={settings.common.timeout}
                                ...
                                <div className="flex space-x-6 pt-2">
                                    <div className="flex-1 max-w-[150px]">
                                        <label className={labelClass}>{t('dolphinScheduler.globalSettings.labels.defaultRetryTimes')}</label>
                                        <input type="number" min="0" value={settings.common.retryTimes}
                                            onChange={e => setSettings({ ...settings, common: { ...settings.common, retryTimes: Number(e.target.value) } })}
                                            className={inputClass}
                                        />
                                    </div>
                                    <div className="flex-1 max-w-[150px]">
                                        <label className={labelClass}>{t('dolphinScheduler.globalSettings.labels.defaultRetryInterval')}</label>
                                        <input type="number" min="1" value={settings.common.retryInterval}
                        ...
                        {activeTab === 'k8s' && (
                            <div className="space-y-5 animate-in fade-in slide-in-from-right-4">
                                ...
                                <div>
                                    <label className={labelClass}>{t('dolphinScheduler.globalSettings.labels.defaultImage')}</label>
                                    <input type="text" value={settings.nodes.k8s.image}
                                ...
                                <div>
                                    <label className={labelClass}>{t('dolphinScheduler.globalSettings.labels.defaultNamespace')}</label>
                                    <input type="text" value={settings.nodes.k8s.namespace}
                        ...
                        {activeTab === 'sql' && (
                            <div className="space-y-5 animate-in fade-in slide-in-from-right-4">
                                ...
                                <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-xs text-blue-700 dark:text-blue-300">
                                    {t('dolphinScheduler.globalSettings.hints.envAutoSelect')}
                                </div>
                                <div>
                                    <label className={labelClass}>{t('dolphinScheduler.globalSettings.labels.defaultDbType')}</label>
                                    <select
                                        value={settings.nodes.sql.dbType || 'MYSQL'}
                                        onChange={e => setSettings({ ...settings, nodes: { ...settings.nodes, sql: { ...settings.nodes.sql, dbType: e.target.value } } })}
                                        className={inputClass}
                                    >
                                        {DB_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
                                    </select>
                                    <p className="text-xs text-slate-500 mt-1">{t('dolphinScheduler.globalSettings.hints.dbType')}</p>
                                </div>
                                <div>
                                    <label className={labelClass}>{t('dolphinScheduler.globalSettings.labels.defaultSqlType')}</label>
                                    <select
                ...
                <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 flex justify-end space-x-3">
                    <button onClick={onClose} className="px-4 py-2 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition-colors text-sm font-medium">
                        {t('common.cancel')}
                    </button>
                    <button onClick={handleSave} className="px-6 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors shadow-sm text-sm font-medium">
                        {t('common.save')}
                    </button>
                </div>
            </div>
