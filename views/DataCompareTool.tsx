...
import { 
  ...
  AlertCircle, ChevronLeft, Plus, X
} from 'lucide-react';
...
export const DataCompareTool: React.FC<{ 
  ...
  const fetchDatabases = async (connId: string, side: 'source' | 'target') => {
    if (isTauri) {
       try {
         const dbs = await window.__TAURI__!.invoke('db_get_databases', { id: connId });
         side === 'source' ? setSourceDbs(dbs as string[]) : setTargetDbs(dbs as string[]);
       } catch(e) {}
    } else {
       // Mock
       const dbs = ['app_production', 'test_db', 'logs'];
  ...
  const fetchTables = async (connId: string, db: string, side: 'source' | 'target') => {
    if (isTauri) {
       try {
         const tables = await window.__TAURI__!.invoke('db_get_tables', { id: connId, db });
         side === 'source' ? setSourceTables(tables as TableInfo[]) : setTargetTables(tables as TableInfo[]);
       } catch(e) {}
    } else {
       // Mock
       const tables = [{name: 'users', rows: 100, size: '1MB'}, {name: 'orders', rows: 500, size: '2MB'}];
  ...
  const fetchColumns = async (connId: string, db: string, table: string) => {
    if (isTauri) {
      try {
        const detail = await window.__TAURI__!.invoke('db_get_table_schema', { id: connId, db, table }) as TableDetail;
      ...
      // Mock
      setAvailableColumns(['id', 'username', 'email', 'status', 'created_at']);
      if (primaryKeys.length === 0) setPrimaryKeys([{ field: 'id', order: 'ASC' }]);
    }
  };
          ...
          <div className="overflow-auto flex-1">
             <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-sm font-mono">
                   {results.map((row, idx) => (
                      <tr key={idx} className={`border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/30 ${
