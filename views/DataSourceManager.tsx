...
import { 
  DatabaseZap, Plus, LayoutGrid, List, Edit, Trash2, X, 
  CheckCircle, AlertCircle, RefreshCw, Power, Server, 
  HardDrive, Layers, Table2, FileCode, Database
} from 'lucide-react';
import { Language, DbConnection, DatabaseType } from '../types';
import { getTexts } from '../locales'; // Import the new locale helper

// Re-using DbViewer's AlertModal for consistency
...
const AlertModal: React.FC<AlertProps> = ({ isOpen, onClose, title, message, type, lang }) => {
  const t = getTexts(lang); // Get current language texts
...
export const DataSourceManager: React.FC<{
  ...
  const handleDeleteClick = (id: string, e: React.MouseEvent) => {
      e.stopPropagation();
      if (window.confirm(t.dataSource.deleteConfirm)) {
          onDelete(id);
      }
  };
  ...
  const handleTest = async () => {
    ...
    if(isTauri) {
      try {
        const success = await window.__TAURI__!.invoke('db_test_connection', { payload: editingConn });
        if (success) {
            setTestStatus('success');
            setAlertState({ isOpen: true, title: t.common.success, message: t.dataSource.connSuccess, type: 'success' });
        } else {
            setTestStatus('failed');
            setAlertState({ isOpen: true, title: t.common.failed, message: t.dataSource.connFailed, type: 'error' });
        }
      } catch (e) {
        setTestStatus('failed');
        setAlertState({ isOpen: true, title: t.common.error, message: String(e), type: 'error' });
      }
    } else {
      setTimeout(() => {
          setTestStatus('failed');
        ...
        {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {connections.map(conn => {
                    ...
                    return (
                        <div key={conn.id} className="group relative bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-md hover:border-blue-400 dark:hover:border-blue-500 transition-all cursor-default">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                    {connections.map(conn => {
                        const style = getDbConfig(conn.type);
                        return (
                            <div key={conn.id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
